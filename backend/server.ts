...
app.post('/player-guesses/start', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    await saveConversationMessage(req.user!.id, result.sessionId, 'system', 'Player-guesses game started');
    res.json(result);
  } catch (error: unknown) {
...
app.post('/player-guesses/question', authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    await saveConversationMessage(req.user!.id, sessionId, 'model', JSON.stringify(result));
...
app.post('/ai-guesses/start', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    await saveConversationMessage(req.user!.id, result.sessionId, 'system', 'AI-guesses game started');
    await saveConversationMessage(req.user!.id, result.sessionId, 'model', JSON.stringify(result.aiResponse));
...
app.post('/ai-guesses/answer', authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    await saveConversationMessage(req.user!.id, sessionId, 'model', JSON.stringify(result.aiResponse));
