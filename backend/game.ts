...
async function startPlayerGuessesGame() {
    ...
    const jsonResponse = await callGeminiAPI<{ secretGame: string }>(initialPrompt);
...
async function handlePlayerQuestion(sessionId: string, userInput: string) {
    ...
    const jsonResponse = await callGeminiAPI<PlayerQAResponse>(prompt, session.chatHistory);
...
async function startAIGuessesGame() {
    ...
    const jsonResponse = await callGeminiAPI<AIJsonResponse>(initialPrompt, chatHistory);
...
async function handleAIAnswer(sessionId: string, userAnswer: string) {
    ...
    const jsonResponse = await callGeminiAPI<AIJsonResponse>(nextTurnPrompt, session.chatHistory);
    session.chatHistory.push({ role: "model", parts: [{ text: JSON.stringify(jsonResponse) }] });
