...
function AuthPage({ onAuth }: AuthPageProps) {
  ...
  const handleGuestAuth = async () => {
    ...
    try {
      ...
      onAuth({ token, username: 'Guest' });
    } catch (err) {
      setError((err as Error).message);
    }
  };

  const handleOIDCAuth = async (credentialResponse: any) => {
    setError(null);

    try {
      const response = await fetch(`${getApiUrl()}/auth/oidc`, {
        method: 'POST',
        headers: {
      ...
      const data: { token?: string; error?: string; username?: string } = await response.json();
      if (!response.ok || !data.token) {
        throw new Error(data.error || 'Unknown error');
      }
      ...
      onAuth({ token: token!, username: username! });
    } catch (err) {
      setError((err as Error).message);
    }
  };
